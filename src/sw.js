...
var files = [
  ...
  'tpls/facturacion/sincronizar.html',
  'tpls/facturacion/emisionFacturas.html',
  'tpls/facturacion/factura.html',
  'tpls/facturacion/listaClientes.html',
  'tpls/facturacion/listaContratos.html',
  'tpls/facturacion/listaFacturas.html',
  'tpls/facturacion/sincronizar.html'
  ...
  event.waitUntil(
    ...
    .then((cache) => {
      ...
      .then(() => {
        ...
        return self.skipWaiting(); //(Para obligar al SW a espera para convertirse en SW activo) To forces the waiting service worker to become the active service worker
      })
      .catch((error) =>  {
        console.log('Error: Fallo el cache', error);
      })
    }).catch(err => console.log("Error insall", err))
  );
...
self.addEventListener('fetch', (event) => {
  ...
  fetch(event.request).then(res => {
    ...
    //window.document.getElementsByTagName("footer")[0].classList.delete("X");
  }).catch(err => {
    //console.log("???????? error del fetch",err)
    //window.document.getElementsByTagName("footer")[0].classList.add("X");
  });
  ...
  event.respondWith(
    ...
    caches.match(request).then((response) => {
      if (response) {
        //console.log("El registro se encuentra en el cache");
        return response;
      }
      ...
      return fetch(request).then((response) => {
        ...
        caches.open(cacheName).then((cache) => {
            cache.put(request, responseToCache).catch((err) => {
              console.log("ERROR DE CODIGO....", err.message);
            });
          }).catch(err => console.log("Cache fetch..", err));
        return response || caches.match("./404.html");
      }).catch(err => console.log("Error de active ", err));
    })
  );
});
